...
* 基于我的协程库[co](https://github.com/Caturra000/co)和`json`库[vsjson](https://github.com/Caturra000/vsJSON)实现（已嵌入），无第三方依赖
...
用法类似`std::thread`的构造函数

参数上建议`by-value`，只要提供`json`构造，都可传入，长度不限

### 连接Endpoint

`Endpoint`就是`asio`里面的`endpoint`，这里作为IP和port的封装
...
序列化用的是`json`，它的性能并不够好，写的`json`库在设计时是为了好用而不是为了高性能（长得像`nlohmann`）

一个能想到的好处就是debug挺方便的，因为你可以直接在`wireshark`里看到传输的是什么

为了尽量弥补序列化问题，我在接口层上都做了简单的协议层抽象，但是这个是局限于编译期，并不能运行时更换

### 标准问题

我之前的项目都是尽可能卡在`C++11`或者`C++14`，同时这里其实`C++17`特性用得不多，也就提供了`optional`（可以写个11/14的兼容类解决）和结构化绑定（完全内部使用，不涉及接口）

虽然没啥理由必须要上17的标准，但是我自己玩的项目为什么不能上17，既然没有不能上的理由，那我就上了，管他的
